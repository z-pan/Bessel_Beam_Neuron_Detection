...
This project implements an automated neuron segmentation pipeline for **Bessel beam two-photon fluorescence microscopy** images of mouse cortical neurons. Each neuron appears as a **horizontally-arranged pair of circular spots** (due to Bessel beam side-lobes), spaced ~25–60 pixels apart (median 43.0 px, measured from 689 confirmed pairs across all 10 datasets). The pipeline processes time-series of single-frame `.tif` files and outputs identified neuron ROIs with confidence scores.
...
### Pair Spacing (FINAL — from 689 confirmed pairs on annotated firing frames)
- **Global: mean=42.9 px, median=43.0, std=8.9, range [25, 60]**
- **Percentiles**: p10=29.8, p25=37.0, p75=50.0, p90=55.0
- Per-dataset means: 39.0 (vol_009) to 44.8 (vol_002) — consistent across resolutions
- The pair spacing does NOT vary with image resolution → fixed property of Bessel beam optics
- **Previous config [30, 50] was TOO NARROW** — would miss pairs at p10 and p90
- **Final config range: [25, 60]** — captures >95% of real pairs
- **Ideal distance for cost function: 43.0 px**
...
2. **Single-spot path** (~52%): Detect individual spots with:
   ...
   - Spatial association: check if position ±43px horizontally has a paired detection in ANY frame (or ±22px from a known pair center)
...
### Spot Properties
- **Spot sigma** (from 3,346 Gaussian fits on real neurons): median=2.52 px, mean=2.72 px, range [0.5, 8.0]
- Paired spots: mean σ=2.90, Single spots: mean σ=2.60
- p10=1.22, p90=4.28 — most spots have σ between 1 and 5 px
- **Revised LoG detection**: `min_sigma=1.0`, `max_sigma=5.0` (covers p10–p90 of measured sigmas)
- **Spot diameter** ≈ 3–9 pixels for typical neurons

### ΔF/F₀ and Local SNR (from 3,202 measurements on annotated firing neurons)
- **ΔF/F₀ at paired spots**: median=0.76, p10=0.21, p25=0.43, p75=1.30, p90=2.04
- **ΔF/F₀ at single spots**: median=0.54, p10=0.13, p25=0.28, p75=1.04, p90=1.66
- **ΔF/F₀ threshold**: `min_peak_dff = 0.20` (10th percentile of paired neurons)
- **Local SNR at paired spots**: median=1.47, p10=0.41, p25=0.76, p75=2.77, p90=4.22
- **Local SNR at single spots**: median=1.03, p10=0.31, p25=0.60, p75=1.79, p90=2.99
- **SNR threshold**: `local_snr_threshold = 0.4` (10th percentile of paired neurons)
- **Key observation**: single-spot neurons have ~30% lower ΔF/F₀ and SNR than paired neurons — consistent with being partially occluded/aberrated

### Estimated Neuron Count per Volume (from spatial clustering of detected positions)
- Range: **27–48 neurons per volume**, mean=34.9, median=32.5
- Per-volume: vol_001=39, vol_002=29, vol_003=34, vol_004=30, vol_005=29, vol_006=31, vol_007=27, vol_008=47, vol_009=35, vol_010=48
- This provides the expected detection count for algorithm validation
...
7. **Pair spacing is universal** — median 43 px across all datasets and resolutions. Config range [25, 60].
...
def find_candidate_pairs(
    ...
    Returns:
        List of (i, j, cost) tuples. Cost is weighted combination of:
            - Normalized distance deviation from ideal_distance (default 43.0 px)
...
The dual-spot pattern arises from the **annular side-lobe structure** of Bessel beams. The two spots are symmetric about the neuron's true center, always aligned along the **horizontal (x) axis** of the scan. **Confirmed spacing: median 43.0 px, range 25–60 px (689 measurements), consistent across all 10 datasets and 5 resolutions**. This is a fixed optical property of the beam, not dependent on image magnification or FOV size.
...
4. **Pair spacing is UNIVERSAL at ~43 px** (median) — confirmed across all 10 datasets and 5 resolutions. Config range [25, 60] captures >95% of real pairs. Do not expect it to vary per dataset.
